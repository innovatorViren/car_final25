{!! Form::open(['route' => 'employee.assign_salesman_update', 'method'=>'Post', 'role' => 'form', 'id' => 'salesmanListForm']) !!}
    ...
    <div class="modal-dialog modal-dialog-scrollable" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">{{ __('employee.select_salesman') }}</h5>
                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            ...
                            @if ($salesmanList->count() > 0)
                                {!! Form::label('assign_salesman',trans("employee.assign_salesman"))!!}
                                <br><br>
                                @php
                                    if(count($subEmployeeId) != count($salesmanIds)){
                                        $checkOrUncheck = '';
                                    }else{
                                ...
                                @foreach ($salesmanList as $row)
                                    @php
                                        if(!in_array($row->id,$subEmployeeId)){
                                            $childSalesman = '';
                                        }else{
                                            $childSalesman = 'checked';
                                        }
                                    ...
                                    <table >
                                        <tr>
                                            <label class="checkbox checkbox-lg">
                                                <input type="checkbox" {{ $childSalesman }} class="checkbox_animated checkRow child-checkbox" name="assign_salesman[]" value="{{ $row->id }}">
                                                <span></span><div class="p-2">{{ $row->first_name . ' - ' .  $row->last_name }}</div>
                                            </label>
                                        </tr>
                                    </table>
                                @endforeach
                            @else
                                <table>
                                    <tr>
                                        <td colspan="8" class="text-center jsNoSalesman"><b>{{ __('employee.no_sales_person_found') }}</b></td>
                                    </tr>
    ...
    var initValidation = function () {
        ...
        $('#salesmanListForm').validate({
            ...
            submitHandler: function (data) {
                var isError = false;
                var id = $(".emp_id").val();
                var emptyCustomerList = $('.jsNoSalesman').text();
