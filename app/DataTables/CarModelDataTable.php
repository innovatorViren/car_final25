...
class CarModelDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        ->addColumn('photo', function ($row) {
                $photoHtml = "";
                $photoHtml = '<div class="left-center">
                    <img class="" src="' . asset('') . '' . $row->model_photo . '" style="max-width:100%;width:100px;min-height:auto;" />
                    </div>';
                return $photoHtml;
            })
        ->rawColumns(['action', 'is_active','photo']);
    }
    ...
    public function checkrights($row)
    {
        ...
        $menu = '';
        $editUrl = route('car-model.edit', [$row->id]);
        $deleteUrl = route('car-model.destroy', [$row->id]);

        if ($user->hasAnyAccess(['users.info', 'car_model.edit', 'car_model.delete', 'users.superadmin'])) {
        ...
        if ($user->hasAnyAccess(['car_model.edit', 'users.superadmin'])) {
        ...
        if ($user->hasAnyAccess(['car_model.delete', 'users.superadmin'])) {
        ...
        if ($user->hasAnyAccess(['users.info', 'car_model.edit', 'country.delete', 'users.superadmin'])) {
            $menu .= "</ul></div></div></td>";
        }
    ...
    {
        $fields = ['car_models.*','car_brands.name as brand_name'];
        $models = CarModel::select($fields)
            ->join('car_brands', function($join){
                $join->on('car_models.car_brand_id', '=', 'car_brands.id');
            });
        if (request()->get('brand')) {
            $models->where('car_brands.name', 'like', "%" . request()->get("brand") . "%");
        }
        if (request()->get('name')) {
            $models->where('car_models.name', 'like', "%" . request()->get("name") . "%");
        }
